        ...
        if (Tools::isConnectedUser()) {
            ...
            if ((0 == $this->teamid) ||
                ...
                if ("addTrack" == $action) {
                    ...
                    // save time track to the database;
                    // this does not yet update the issue
                    $trackid = TimeTrack::create($userid, $defaultBugid, $job, $timestamp, $duration);

                    // MBU  don't change the backlog at all here, since this
                    // will always be handled via the time_tracking_ajax.php;
                    // otherwise we get unwanted updates in the issue history
                    // if the backlog is not updated: CodevTT Backlog 0 => 0

                    // MBU as a speciality, we have one specific issue status,
                    // which will not update the backlog as well: '100:ongoing';
                    ...
                    if (($job != $job_support)
                        ...
                        // MBU this really is one custom field too much;
                        // why not simply change the existing estimation
                        $totalEE = ($issue->getEffortEstim() + $issue->getEffortAdd());

                        // note: if backlog is NULL, the values to propose in the
                        // dialog box are not the ones used for ProjectManagement
                        $backlog = $issue->getBacklog();
                        if (!is_null($backlog) && is_numeric($backlog)) {
                            // normal case
                            $drift = $issue->getDrift();
                        } else {
                            // MBU move this into the issue class
                            // and provide the backlog as argument
                ...
                elseif ("deleteTrack" == $action) {
                    ...
                    try {
                        ...
                        if ($job != $job_support
                            ...
                            // MBU there is a problem here; if you create
                            // some time track without updating the backlog
                            // because -- you maybe realised, that it's more
                            // work than you thought, then removing this time
                            // track should not increase the backlog; users
                            // generally understand the remove time track as
                            // an UNDO operation -- but it actually is not;
